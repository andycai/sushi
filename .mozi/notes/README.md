...
这里存放一类设计文档。**Agent Note** 记录影响本代码库的决策或提案：代码和文档无法承载的*为什么*以及*放弃了什么*。本文件是 Agent Note 路径、生命周期、分类、Markdown-only 格式和归档规则的唯一权威规范；各层 `AGENTS.md` 只补充代理操作约束，不得与本文件重复定义或冲突。
...
- **生命周期**（顶层文件夹）是 Agent Note 的状态，Agent Note 随状态变化在文件夹之间移动：
  ...
  - **`rejected/`**：提案经过讨论后被否决。仅当其决策依据仍能避免一种诱人且影响重大的错误时保留；否则删除该 Markdown 文件。
...
每份 Agent Note 属于以下表格中的编码类别；验证器拒绝其他文件夹。新增类别需要同时更新验证器闭集与本节。
...
当一份 implemented Agent Note 记录的交付决策已经完整落地，且其决策依据不太可能再指导未来工作时，将其归档。如果其中的备选方案、归属边界、否定性保证、持久化语义或协议语义、安全规则，或者重新引入条件仍有价值，则继续作为活跃记录保留。绝不归档 proposed Agent Note：过时的提案应转为 rejected。仅当 rejected Agent Note 仍能避免一种可能发生的错误时保留；否则删除该 Markdown 文件。所有取代检查、迁移和归档都通过唯一入口技能 `mozi-agent-notes` 完成，不要根据字数、存续时间或目标配额来判断。

归档路径编码为 `archived/{class}/yyyy-mm-dd-topic-title.md`；其中有意省略 `implemented`，因为只有 implemented Agent Note 可以进入归档。归档变更会移动 Markdown 文件，保留 `Status: implemented`，在 Note 中紧接插入 `Archived: YYYY-MM-DD` 行，并修复或删除活跃文档中的入站链接。归档时只允许对内容做这些更改。

封存后，每份归档文件都永久冻结。禁止编辑、翻译、重新格式化、更新、移动或删除，也不得将其视为当前行为的权威依据。普通验证默认跳过归档正文；显式 `--include-archive` 只检查归档路径、状态和归档日期，不检查其中的出站链接。当活跃文档有意引用历史时，仍可链接到归档 Agent Note。

## 何时需要写一份

每个非平凡变更都必须在同一变更中新增或更新至少一份 Agent Note。如果变更修改了行为、架构、跨文件或跨包约定、流程或工具、测试策略、磁盘存储格式、协议格式（wire format）或配置格式，或者维护者可能合理重新审视的其他决策，就属于非平凡变更。对未来重大工作的提案从 `proposed/` 开始；已经做出的决策从 `implemented/` 开始。选择与决策匹配的类别文件夹（见[分类](#classification)）。
...
被完全取代的 implemented Agent Note 可以合并到当前持有该决策的记录中，并删除原文件。删除前，当前记录必须保存所有独有的决策依据、备选方案、影响、必需的验证和明确指出的覆盖缺口；修复所有入站链接。仅部分被取代的记录不符合此条件：保留两个记录并让它们互相链接，同时更新所有仍然适用的事实。合并不得将旧文件改写成与其相反的决策，也不得让 git 历史成为决策依据的唯一副本。
...
后跟一个空行。`Status:` 的值有三种形式，且必须与文件所在的生命周期文件夹一致；验证器会交叉检查：
