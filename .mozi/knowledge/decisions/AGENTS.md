...
本文件对整个 `.mozi/knowledge/decisions/` 目录树生效，并继承[仓库根 `AGENTS.md`](../../../AGENTS.md)。Agent Note 本质上是由代理编写的 RFC：用于长期保留提案和决策记录，说明决策依据、曾考虑的替代方案、后果以及所需的验证。[README.md](README.md) 是路径、生命周期、分类和文件格式的唯一权威规范；本文件只规定代理在决策树中的通用操作。
